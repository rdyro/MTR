...
def parse_config():
    ...
    parser.add_argument(
        "--without_sync_bn", action="store_true", default=False, help="whether to use sync bn"
    )
    parser.add_argument("--fix_random_seed", action="store_true", default=False, help="")
    parser.add_argument(
        "--ckpt_save_interval", type=int, default=2, help="number of training epochs"
    )
    parser.add_argument(
        "--local-rank", type=int, default=None, help="local rank for distributed training"
    )
    parser.add_argument(
        "--max_ckpt_save_num", type=int, default=5, help="max number of saved checkpoint"
    )
    parser.add_argument(
...
def build_scheduler(
    optimizer, dataloader, opt_cfg, total_epochs, total_iters_each_epoch, last_epoch
):
    decay_steps = [
        x * total_iters_each_epoch for x in opt_cfg.get("DECAY_STEP_LIST", [5, 10, 15, 20])
    ]
...
def main():
    ...
    cfg.ROOT_DIR = Path(__file__).parent
